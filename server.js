...
app.post('/api/recommend', async (req, res) => {
  try {
    ...
    console.log(`\n🔍 RAG Recommend: "${searchQuery}"`);
    
    // For now, redirect to smart-search with enhanced features
    // This will use the existing smart search with improved logic
    req.body.inquiry_text = searchQuery;
    
    // Call the smart-search handler
    const smartSearchHandler = async () => {
      // Use existing smart search logic
      const response = await fetch(`http://localhost:${PORT}/smart-search`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ inquiry_text: searchQuery })
      });
      ...
      if (result.success) {
        result.method = result.method || 'smart_search_gpt4';
        result.search_info = {
          retrieval_method: result.candidates_evaluated ? 'hybrid' : 'semantic',
          llm_used: true,
    ...
    // Try to use existing smart search first
    try {
      // Direct call to smart search logic
      const { inquiry_text } = req.body;
      
      // Use existing findSemanticMatches and GPT-4 validation
      if (!openaiAvailable || !openai) {
      ...
      if (embeddingsReady) {
        candidates = await findSemanticMatches(inquiry_text, 0.35, 8);
        console.log(`📊 Found ${candidates.length} semantic candidates`);
      }
      
      // Add text-based matches if needed
      if (candidates.length < 8) {
        const textMatches = findTextMatches(inquiry_text, 0.15, 10);
        const existingIds = new Set(candidates.map(c => c.case_id));
        const newMatches = textMatches.filter(m => !existingIds.has(m.case_id));
        candidates = [...candidates, ...newMatches].slice(0, 10);
      }
      ...
      res.json({
        success: true,
        inquiry: inquiry_text,
        answer: candidates[0]?.response_text || 'לא נמצאה תשובה מתאימה',
        confidence: 0.8,
        sources: candidates.slice(0, 3).map(c => ({
          case_id: c.case_id,
          row_number: c.row_number,
          relevance: Math.round((c.similarity || 0.5) * 10)
        })),
        method: 'rag_hybrid',
        candidates_evaluated: candidates.length,
        search_info: {
          retrieval_method: embeddingsReady ? 'semantic' : 'text',
          llm_used: false,
          timing_ms: Date.now() - startTime
        }
      });
...
// NEW: RAG Status endpoint (simplified version of debug/status)
app.get('/api/status', (req, res) => {
  ...
  res.json({
    ...
    index_pack: {
      exists: municipalData.length > 0,
      loaded_in_cache: true,
      document_count: municipalData.length,
      embeddings_count: embeddingsCount,
      row_count: municipalData.length
    },
    features: {
